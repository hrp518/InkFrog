...
static void EPD_3IN52_lut_DU(void);   /* 前向声明: 定义在文件后部 */
void EPD_3IN52_Display(void)
{
    ...
    // 步骤4: 触发刷新 (统一改用 DU 快刷, 避开 GC LUT 反极性带来的各种问题)
    printf("[EPD_Display] Step 4: Triggering DU refresh...\r\n");
    EPD_3IN52_lut_DU();
    EPD_3IN52_refresh();
    OS_MSleep(2);
...
// 'C' - 16x16
static const uint8_t font_C[32] = {
    0x00,0x00, 0x00,0x00, 0x3E,0x00, 0x63,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00,
    0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0x63,0x00, 0x3E,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00
};
// 'h' - 16x16
static const uint8_t font_h[32] = {
    0x00,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xDC,0x00, 0xE6,0x00, 0xC3,0x00, 0xC3,0x00,
    0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00
};
// 'g' - 16x16
static const uint8_t font_g[32] = {
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x3E,0x00, 0x63,0x00, 0xC3,0x00, 0xC3,0x00,
    0xC3,0x00, 0xC3,0x00, 0x7F,0x00, 0x03,0x00, 0x03,0x00, 0x3E,0x00, 0x00,0x00, 0x00,0x00
};
// 'I' - 16x16
static const uint8_t font_I[32] = {
    0xFF,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00,
    0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0xFF,0x00
};
// 'n' - 16x16
static const uint8_t font_n[32] = {
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0xDE,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00,
    0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0x00,0x00
};
// 'k' - 16x16
static const uint8_t font_k[32] = {
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0xC1,0x00, 0xC2,0x00, 0xC4,0x00, 0xC8,0x00,
    0xD0,0x00, 0xC8,0x00, 0xC4,0x00, 0xC2,0x00, 0xC1,0x00, 0xC0,0x00, 0x00,0x00, 0x00,0x00
};
// 'f' - 16x16
static const uint8_t font_f[32] = {
    0x1C,0x00, 0x1C,0x00, 0x18,0x00, 0x1C,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00,
    0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00
};
// 'b' - 16x16
static const uint8_t font_b[32] = {
    0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xFC,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00,
    0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xFF,0x00, 0x00,0x00, 0x00,0x00
};
// 'i' - 16x16
static const uint8_t font_i[32] = {
    0x30,0x00, 0x00,0x00, 0x00,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00,
    0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x30,0x00, 0x00,0x00
};
// '.' - 16x16
static const uint8_t font_dot[32] = {
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00,
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x30,0x00, 0x30,0x00, 0x00,0x00
};

/* Draw 16x16 character at position */
// 't' - 16x16 (booting... 的 t 原本缺失, 显示成空格)
static const uint8_t font_t[32] = {
    0x00,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x7E,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00,
    0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x18,0x00, 0x3C,0x00, 0x00,0x00, 0x00,0x00
};
// 'p' - 16x16 (preparing 用; 碗形 + 下伸部)
static const uint8_t font_p[32] = {
    0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0xFC,0x00, 0xC6,0x00, 0xC3,0x00, 0xC3,0x00,
    0xC3,0x00, 0xC3,0x00, 0xC3,0x00, 0xC6,0x00, 0xFC,0x00, 0xC0,0x00, 0xC0,0x00, 0x00,0x00
};
// 'F' - 16x16 (TTF Font 用)
static const uint8_t font_F[32] = {
    0x00,0x00, 0xFF,0x00, 0xFF,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xC0,0x00, 0xFE,0x00,
...
/* Draw 16x16 character scaled by `scale` (1 = original, 2 = 2x ...) */
static void EPD_DrawChar16Scaled(uint16_t x, uint16_t y, const uint8_t* bitmap, uint8_t scale)
{
    for (int row = 0; row < 16; row++) {
            ...
            if (byte & (0x80 >> (col % 8))) {
                for (int sy = 0; sy < scale; sy++)
                    for (int sx = 0; sx < scale; sx++)
                        EPD_SetPixel(x + col * scale + sx, y + row * scale + sy, 0);
            }
        }
    }
}

/* 在指定 y 处水平居中绘制一串字符, 支持放大 (scale) 与字距 (advance, 最终像素) */
void EPD_DrawStringScaled(const char *text, uint16_t y, uint8_t scale, uint16_t advance)
{
    int len = strlen(text);
    int total_w = (len - 1) * advance + 16 * scale;
    int start_x = (240 > total_w) ? (240 - total_w) / 2 : 0;
    for (int i = 0; i < len; i++)
        EPD_DrawChar16Scaled(start_x + i * advance, y, EPD_GetCharBitmap(text[i]), scale);
}
